...
object Classpath {
  ...
  private lazy val build: (ServletContext, String, Set[String]) => Classpath = (klass, relativeJarPath, libs) => new Classpath(klass, relativeJarPath, libs)

  def apply(context: ServletContext, relativeJarPath: String, libs: Set[String] = Set()) = build(context, relativeJarPath, libs)
}
 ...
 */
class Classpath(context: ServletContext, relativeJarPath: String, libs: Set[String] = Set()) {
