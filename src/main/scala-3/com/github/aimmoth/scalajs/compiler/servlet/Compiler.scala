...
class Compiler(compilerClassPath: Classpath, env: String) { self =>
  ...
  def compile(src: CompileActor.Source, logger: String => Unit = _ => ()): Option[(Seq[MemIRFileImpl], Seq[String])] = {
    ...
    val settingsWithOutputWithJs = settings.scalajs.updateIn(settingsWithOutput, true)
    ...
    if (vd.iterator.isEmpty)
      ...
      def findFilesWithExtension(vd: AbstractFile, ext: String) = 
      ...
      val sjsirFiles =
        findSjsirFiles(vd).map {
          case x => new MemIRFileImpl(x.path, None, x.toByteArray)
        }.toSeq
  ...
  def `export`(output: MemOutputDirectory): String = {
    ...
    new String(output.content("main.js").get)
  }
  ...
  def link(userFiles: Seq[MemIRFileImpl], mainClasses: Seq[String], fullOpt: Boolean): MemOutputDirectory = {
    val moduleInitializers = mainClasses.map(ModuleInitializer.mainMethodWithArgs(_, "main"))
    println(moduleInitializers)
